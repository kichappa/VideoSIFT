...
function findBlobs(img_gpu, height, width, imgWidth, octaves, scales=5, sigma0=1.6, k=-1)
    if k == -1
        k = 2^(1 / (scales - 3))
    end
    time_taken = 0
    ...
    for octave in 1:octaves
        for layer in 1:scales
            ...
            if cld(height, prod(threads_column)) >= 1
                ...
                time_taken += CUDA.@elapsed buffer .= 0
                gaussian_kernel_gpu = CuArray(getGaussianKernel(2 * apron + 1, sigma))
                if layer == 1
                    if octave == 1
                        # take the image as input
                        time_taken += CUDA.@elapsed @cuda threads = threads_column blocks = blocks_column shmem = shmem_column maxregs = 64 col_kernel_strips(img_gpu, gaussian_kernel_gpu, buffer, Int32(width), Int16(height), Int8(apron))
                    else
                        # take the previous octave's third last output as input
                        time_taken += CUDA.@elapsed @cuda threads = threads_column blocks = blocks_column shmem = shmem_column maxregs = 64 col_kernel_strips(out_gpus[octave-1][layers-2], gaussian_kernel_gpu, buffer, Int32(width), Int16(height), Int8(apron))
                    end
                else
                    # take the previous layer's output as input
                    time_taken += CUDA.@elapsed @cuda threads = threads_column blocks = blocks_column shmem = shmem_column maxregs = 64 col_kernel_strips(out_gpus[octave][layer-1], gaussian_kernel_gpu, buffer, Int32(width), Int16(height), Int8(apron))
                end
                time_taken += CUDA.@elapsed @cuda threads = threads_row blocks = blocks_row shmem = shmem_row maxregs = 32 row_kernel(buffer, gaussian_kernel_gpu, out_gpus[octave][layer], Int16(height - 2 * apron), Int16(height), Int32(width), Int16(imgWidth), Int8(apron))
            end
        end
    end
end
...
#     schemaBase = DAict(:name => "gaussian1D", :epsilon => 0.1725)
