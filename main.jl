...
function getGPUElements(img, height, width, layers, octaves, sigma0 = 1.6, k = -1)
	...
	return CuArray(img), out_gpu, DoG_gpu, conv_gpu, CUDA.zeros(Float32, height, width)
end
	...
	for octave in 1:octaves
		for layer in 1:scales
			...
			if cld(height, prod(threads_column)) >= 1
				...
				println("O$(octave)L$(layer): height: $height, width: $width, imgWidth: $imgWidth, apron: $apron, acc_ap: $(accumulative_apron), res_ap: $resample_apron")
			end
		end
		height = height ÷ 2
		width = width ÷ 2
...
let
	...
	img_gpu, out_gpu, DoG_gpu, convolution_gpu, buffer = getGPUElements(img, height, width, layers, octaves, sigma0, k)
	iterations = 1
	...
	for j in 1:octaves
		for i in 1:2
			max = maximum(DoG_gpu[j][i])
			println("O$(j)L$(i): max: $(maximum(DoG_gpu[j][i])), min: $(minimum(DoG_gpu[j][i]))")
			if max .!= 0
				save("assets/DoG_nov24_o$(j)l$(i).png", colorview(Gray, Array(DoG_gpu[j][i] / maximum(DoG_gpu[j][i]))))
			else
