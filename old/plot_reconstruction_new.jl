...
# ╔═╡ 1b6969f6-086f-11f0-386c-293ce05c2161
# ╠═╡ show_logs = false
begin
	ENV["JULIA_CONDAPKG_BACKEND"] = "Current"
	ENV["JULIA_PYTHONCALL_EXE"] = expanduser("~/.conda/envs/cv/bin/python")
	using PythonCall, CondaPkg
	using VideoIO, Images, FileIO, Glob, JLD2, UnPack, CSV, DataFrames, LinearAlgebra, Hungarian, OffsetArrays
	using Plots
	using PlutoUI
	using Statistics, CUDA, NNlib, CUDA.CUSOLVER
end

# ╔═╡ d9f481a4-d0c8-4da3-b6fd-0b7f2282b821
using PlutoPlotly

# ╔═╡ a5c99c67-bb24-48e1-810a-386a7dd41fe9
begin
	include("triangulate.jl")
	include("calibration_helper.jl")
end

# ╔═╡ e28a4003-eb1d-4692-a2d8-de5c060af8e9
html"""
<style>
  main {
	max-width: 50vw;
    /* align-self: flex-mid; */
    /* margin-right: max(max(0px, 50% - 365.5px), min(100% - 731px, 500px)); */
  }
...
# ╔═╡ 898dfd2b-42f3-4338-9964-f955eb92d7c7
begin
	ENV["JULIA_CONDAPKG_BACKEND"] = "Current"
	ENV["JULIA_PYTHONCALL_EXE"] = expanduser("~/.conda/envs/cv/bin/python")
end

# ╔═╡ cd7b922f-9f5c-4991-a508-3d981fcb9954
# ╠═╡ show_logs = false
begin
	try
		CondaPkg.add("opencv")
...
# ╔═╡ 635ef4fe-b53c-4bd4-8f34-43935f343700
begin
	K1, d1, R1, t1 = extract_matrices(1, "MVI_1171_69.png", "../assets/videos/cam/calibration/data_3_4_3.jld2")
	K2, d2, R2, t2 = extract_matrices(2, "DSC_0264_822.png", "../assets/videos/cam/calibration/data_3_4_3.jld2")
	...
	for i in 1:size(a1, 1)
		for j in 1:size(a2, 1)
			d[i, j] = distance3D(K1, d1, R1, t1, K2, d2, R2, t2, a1[i, :], a2[j, :])
		end
	end
	
	assignment, _ = hungarian(d)
	
	assignment_arr = zeros(Integer, size(a, 1)÷2, size(a, 1)÷2)
	for i in 1:size(assignment, 1)
		assignment_arr[i, assignment[i]] = 1
	end
	
	reconstructed_points = zeros(Float32, size(a1, 1), 3)
	for i in 1:size(assignment, 1)
		# calculate the intersection point
		intersection = intersection3D(K1, d1, R1, t1, K2, d2, R2, t2, a1[i, :], a2[assignment[i], :])
		reconstructed_points[i, :] = intersection
	end
	# plotReconstructedPoints(reconstructed_points)
end

# ╔═╡ a1d9fcbf-b9a4-401a-a24d-1435f68b2874
...
let	
	...
	layout = Layout(
	        title = "Reconstructed 3D Points - To Scale",
	        scene = attr(
...
# ╔═╡ 028e84f0-eb56-4370-a981-7b679f396e2f
a

# ╔═╡ ffd79ef6-7e13-4012-9479-86d8a2404fb3
begin
	# K1, d1, R1, t1 = extract_matrices(1, "MVI_1171_69.png", "../assets/videos/cam/calibration/data_3_4_3.jld2")
	# K2, d2, R2, t2 = extract_matrices(2, "DSC_0264_822.png", "../assets/videos/cam/calibration/data_3_4_3.jld2")

	a_points = zeros(2, size(a,1)÷2, 2)
	a_points[:, :, 1] = a[begin:size(a,1)÷2, :]'
	a_points[:, :, 2] = a[size(a,1)÷2+1:end, :]'	

	
	frames5 = ["MVI_1171_69.png", "DSC_0264_822.png"]
	cameras5 = [1, 2]

	N = size(a_points, 2) # number of points
	M = size(a_points, 3) # number of views
...
let
	Ks = Any[[1415.041973954641 0.0 916.3298221454788; 0.0 1414.6019518351 547.343118338264; 0.0 0.0 1.0], [1525.9681369704565 0.0 1007.2268419349415; 0.0 1529.0418661707852 523.4928703153819; 0.0 0.0 1.0]]
	Rs = Any[[-0.9805328365146804 0.01017581195682355 -0.19609132914918642; 0.08064590648605288 0.9314091659686035 -0.3549270394274213; 0.17902959052491302 -0.3638315797188183 -0.9140979090424566], [0.8553538704422734 0.5086789210358897 0.09805871513121989; 0.4485764103568589 -0.6325815043882437 -0.6313634803956127; -0.25913116445301915 0.5840260230500602 -0.7692624025711597]]
	ts = Any[[-194.73019206620688, -147.7311043064236, 1287.5892235577517], [-56.30203516467598, 176.432419590241, 1400.1852186874537]]
...
# ╔═╡ 168f8393-380f-4d2e-b727-8703a6054fee
md"![Unable to load the image, imagine two checkerboards](https://gtvault-my.sharepoint.com/personal/kshenoy8_gatech_edu/Documents/Documents/Courses/CS%206476%20CV/Peoject/assets/videos/cam5/corners/20250402_163250_803_corners.png?Web=1)"

# ╔═╡ e409c4a1-f564-49b4-980a-9b1fdeff0b56
# ╠═╡ show_logs = false
let
	cv = pyimport("cv2")
	np = pyimport("numpy")
	ret, corners = findChessboardCorners(["..\\assets\\videos\\cam5\\20250402_163250_800.png", "..\\assets\\videos\\cam5\\20250402_163250_239.png", "..\\assets\\videos\\cam5\\20250402_163250_100.png"], (5, 8); cv=cv, np=np)
	
	d, assignments, reconstructed_points = triangulatePoints(corners, ["cam5\\20250402_163250_800.png", "cam5\\20250402_163250_239.png", "cam5\\20250402_163250_100.png"], "..\\assets\\videos\\cam\\calibration\\data_5_big.jld2");
end

# ╔═╡ a492d2a2-f293-4747-9e5c-1ff289c81778
begin
	path = "../assets/videos/cam14"
	f = vcat(glob("*.mp4", path), glob("*.mov", path), glob("*.MP4", path), glob("*.MOV", path))[1]
	md"Seekbar: $(@bind frame Slider(1:VideoIO.get_number_frames(f), show_value=true))"
end

# ╔═╡ c6726264-378f-4002-b0c2-5ede776d53d7
begin
	...
	function getFrame(f::String, frame, fps=25)
...
# ╔═╡ 898d276c-7fe0-420b-9497-4f0aee1bac84
begin
	v = VideoIO.openvideo(f)
	getFrame(v, frame)
end

# ╔═╡ f0460d8c-1692-4943-8d34-1117550cf9b0
begin
	cv = pyimport("cv2")
	np = pyimport("numpy")
end

# ╔═╡ f2b0119f-3f1b-4b4d-893a-1c2e50a7ad20
begin
	frame1 = 277
	frame2 = 388
	frames = [277, 124, 12]
	cameras = [1, 1, 1]
	ret, corners = findChessboardCorners(map(x->getFrame(v, x), frames), (11, 8); cv=cv, np=np, invert=true)

	
	_, _, reconstructed_points1 = triangulatePoints(corners, map(x->"$x", frames), cameras, "../assets/videos/cam/calibration/data_8.jld2");
end

# ╔═╡ 1d3e3788-369b-4ed6-bb54-d7ae9228ce5f
findChessboardCorners(map(x->getFrame(v, x), [277, 124, 12])[1], (11, 8); cv=cv, np=np, invert=true)
...
begin
	...
	md"""Plot camera pairs: $(@bind set2 Select(map(x->Pair(set2[x], names[x]), axes(set2, 1))))"""
end

# ╔═╡ 291a3f7c-1b42-45b1-bd46-9d7ade4b0fcf
let
	set2 = ["", "_PnP_refine", "_RO_refine"]
	names = ["RO", "PnP + refine", "RO + refine"]
...
let
	...
	jld_file = "../assets/videos/cam/calibration/data_12$set2.jld2"
	@unpack ret_arr, mtx_arr, dist_arr, rvecs_arr, tvecs_arr, filenames = jldopen(jld_file)
	...
	# Compute common axis limits
	# all = hcat(reconstructed_points1[:, :, 1], camera_pos)
	all = hcat(camera_pos)
    xmin, xmax = extrema(all[1, :]./1000)
    ymin, ymax = extrema(all[2, :]./1000)
    zmin, zmax = extrema(all[3, :]./1000)
    ...
    trace_cam = PlutoPlotly.scatter3d(
	        x = camera_pos[1, :]./1000,
	        y = camera_pos[2, :]./1000,
	        z = camera_pos[3, :]./1000,
	        mode = "markers",
	...
	layout = Layout(
	        title = "New Reconstructed - All camera positions",
	    #     scene = attr(
		   #      xaxis = attr(title = "X", range = [(xmin+xmax)/2 - 1.1 * range/2, (xmin+xmax)/2 + 1.1 * range/2]),
		   #      yaxis = attr(title = "Y", range = [(ymin+ymax)/2 - 1.1 * range/2, (ymin+ymax)/2 + 1.1 * range/2]),
		   #      zaxis = attr(title = "Z", range = [(zmin+zmax)/2 - 1.1 * range/2, (zmin+zmax)/2 + 1.1 * range/2])
...
let
	path = "../assets/videos/cam$cam_num"
	f = vcat(glob("*.mp4", path), glob("*.mov", path), glob("*.MP4", path), glob("*.MOV", path))[1]
	getFrame(f, cam_frame)
end

# ╔═╡ 83515a7c-16e6-4ebb-b535-b1980a565dae
let
	jld_file = "../assets/videos/cam/calibration/data_$(cam_num)_vid.jld2"
	@unpack ret_arr, mtx_arr, dist_arr, rvecs_arr, tvecs_arr, filenames = jldopen(jld_file)
	...
	camera_pos = zeros(3, size(tvecs_arr[1], 2))
	for i in axes(camera_pos, 2)
		camera_pos[:, i] = -rvecs_arr[1][i-1] * tvecs_arr[1][:,i]
		# camera_pos[:, i] = -tvecs_arr[1][:,i]
	end

	
    trace_cam = PlutoPlotly.scatter3d(
	        x = camera_pos[1, cam_frame - 1: cam_frame + 1]./1000,
	        y = camera_pos[2, cam_frame - 1: cam_frame + 1]./1000,
	        z = camera_pos[3, cam_frame - 1: cam_frame + 1]./1000,
	        mode = "markers",
	        marker = attr(color = "red", size = 3),
	        name = "Camera Centres"
	    )

	trace_cam_full = PlutoPlotly.scatter3d(
		x = camera_pos[1, :]./1000,
		y = camera_pos[2, :]./1000,
		z = camera_pos[3, :]./1000,
		mode = "markers",
	...
	cb_plane = (-1, 1)
	cb_size = 16
	cb_points = zeros(Float32, 3, prod(cb_grid))
	
	cb_points[1, :] = reshape([sign(cb_plane[1]) * x * cb_size for x in 0:(cb_grid[1]-1), y in 0:(cb_grid[2]-1)], 1, :)
	cb_points[2, :] = reshape([sign(cb_plane[2]) * y * cb_size for x in 0:(cb_grid[1]-1), y in 0:(cb_grid[2]-1)], 1, :)
	
	trace_checker = PlutoPlotly.scatter3d(
	        x = cb_points[1, :]./1000,
	        y = cb_points[2, :]./1000,
	        z = cb_points[3, :]./1000,
	        mode = "markers",
	...
	# Compute common axis limits
	# all = hcat(reconstructed_points1[:, :, 1], camera_pos)
	all = hcat(camera_pos, cb_points)
    xmin, xmax = extrema(all[1, :]./1000)
    ymin, ymax = extrema(all[2, :]./1000)
    zmin, zmax = 0, maximum(all[3, :]./1000)
	range = max(xmax-xmin, ymax-ymin, zmax-zmin)
	...
	plt = Plot([trace_cam, trace_checker, trace_cam_full], layout)
	PlutoPlotly.plot(plt)
end
...
let
	...
	# trace_select_frames = PlutoPlotly.scatter3d(
	#         x = camera_pos[1, [frame1, frame2]]./1000,
	#         y = camera_pos[2, [frame1, frame2]]./1000,
	#         z = camera_pos[3, [frame1, frame2]]./1000,
	#         mode = "markers",
	#         marker = attr(color = "red", size = 2),
	#         name = "Camera Centres"
	#     )
	# trace_points = PlutoPlotly.scatter3d(
	#         x = reconstructed_points1[1, :, 1]./1000,
	#         y = reconstructed_points1[2, :, 1]./1000,
	#         z = reconstructed_points1[3, :, 1]./1000,
	#         mode = "markers",
	#         marker = attr(color = "blue", size = 2),
	#         name = "Triangulation"
	# )
	...
	plt = Plot([trace_cam, trace_checker], layout)
	PlutoPlotly.plot(plt)
end

# ╔═╡ 6d12f950-7c05-4dee-9f15-3caff69b8db1
md"""Plot camera pairs: $(@bind set Select(map(x->Pair(x, (x, x % size(reconstructed_points1, 3) + firstindex(reconstructed_points1))), axes(reconstructed_points1, 3))))"""

# ╔═╡ 868d7f75-32d7-41c7-95aa-fb3da2b9e833
let
    ...
    xmin, xmax = extrema(reconstructed_points1[1, :, set]./1000)
    ymin, ymax = extrema(reconstructed_points1[2, :, set]./1000)
    zmin, zmax = extrema(reconstructed_points1[3, :, set]./1000)
	range = max(xmax-xmin, ymax-ymin, zmax-zmin)
 ...
 #    zmin, zmax = extrema(all[3, :]./1000)
	...
	trace_points = PlutoPlotly.scatter3d(
	        x = reconstructed_points1[1, :, set]./1000,
	        y = reconstructed_points1[2, :, set]./1000,
	        z = reconstructed_points1[3, :, set]./1000,
	        mode = "markers",
	        marker = attr(color = "blue", size = 2),
	        name = "Triangulation"
	)
	...
	trace_select_frames = PlutoPlotly.scatter3d(
	        x = camera_pos[1, set]./1000,
	        y = camera_pos[2, set]./1000,
	        z = camera_pos[3, set]./1000,
	        mode = "markers",
	        marker = attr(color = "red", size = 2),
	        name = "Camera Centres"
	    )
	...
	plt = Plot([trace_points, trace_select_frames], layout)
	PlutoPlotly.plot(plt)
end
...
manifest_format = "2.0"
project_hash = "46447467be84b5937b4816894c4fa6720feb2bed"
...
git-tree-sha1 = "4f37de70062a474e0f4edc50d1bb6ebfacbf03c2"
