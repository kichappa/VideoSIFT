...
Feature detection over a scale space is a workhorse of computer vision, and the
Scale-Invariant Feature Transform (SIFT)~\cite{lowe2004} is its most widely used
instance. Its computational profile is a poor match for the way it is usually
implemented on GPUs. A canonical pipeline builds a Gaussian pyramid, subtracts
adjacent scales to form a Difference-of-Gaussians (DoG) volume, and then
searches that volume for local extrema before it assigns orientations and emits
descriptors. Each stage is usually realized as a separate kernel that reads and
writes large intermediate buffers to device memory. On modern GPUs the off-chip
memory bandwidth is far smaller than the available arithmetic throughput, so the
cost is dominated by materializing the DoG volume and by streaming scale-space
images through DRAM rather than by the convolutions or the comparisons
themselves.

This mismatch becomes more severe for real-time video. A detector must run on
every frame within a fixed latency budget, and per-frame memory allocation
together with pyramid teardown adds avoidable overhead. Existing GPU
implementations such as SiftGPU~\cite{siftgpu} and PopSift~\cite{popsift} reach
strong throughput, yet they keep the staged, memory-materializing structure
inherited from CPU designs. They also target the classic descriptor rather than
the specific needs of dense deformation measurement.

We present \textbf{SIBD}, a \emph{Scale-Invariant Blob Detector} that is fully
GPU-resident. The entire pipeline runs from separable Gaussian smoothing to a
compacted list of characterized keypoints without returning control to the host
and without ever writing a DoG volume to device memory. SIBD makes several
systems observations concrete. The DoG difference and the $3\times3\times3$
scale-space extremum test are fused in shared memory, so that a single kernel
loads a small stack of adjacent Gaussian scales into on-chip memory, forms the
differences in place, performs the min/max neighborhood test, and never spills
the DoG. The separable convolution is tiled with overlapping \emph{aprons} so
that halo regions are resolved inside a thread block, and the column and row
passes are fused into a single launch. The sparse extrema are packed into a
dense keypoint list with \emph{warp-aggregated} ballot-and-atomic compaction,
which avoids a full-image scan on the host.

Beyond this systems structure, SIBD introduces a keypoint characterization
tailored to blobs. In place of Lowe's histogram of gradient \emph{directions},
SIBD accumulates an angular profile of the \emph{radial intensity derivative}
around each keypoint, binned by angular position. The coefficient of variation
of this profile is a direct and rotation-symmetric measure of how blob-like a
candidate is, and we use it in place of SIFT's Hessian-based edge rejection as a
single symmetry filter that is cheap to compute. This choice is well matched to
our target application, which is the tracking of printed or painted \emph{blob
markers} on skin as an anatomical joint flexes and the recovery of a dense
\emph{strain field} from the resulting position field.
...
SIFT~\cite{lowe2004} established the Gaussian and DoG scale space together with
the keypoint localization and descriptor stages that most later detectors
refine. SURF~\cite{surf} approximates the Laplacian
...
which augments Eq.~(1) of the Oral-Qual writeup in several respects
...
\qquad \rho>0 .
\end{equation}
This is the (inward) \enrich{radial \emph{derivative}} of intensity, that is, a
...
particular its mean and its standard deviation, and therefore the coefficient of variation
...
This single scalar test replaces SIFT's two-part refinement, namely the
low-contrast threshold on the DoG response together with the Hessian
principal-curvature ratio $\mathrm{tr}(H)^2/\det(H)$ used for edge
rejection~\cite{lowe2004}. \enrich{In
...
like Lowe's, so state that clearly. Shared-memory capacity bounds patch radius / layer
