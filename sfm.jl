...
let
	...
	for frame in sfm_start:nImages:sfm_end
		...
		for i in frame:min(frame+nImages-1, sfm_end)
			if isnothing(imgs)
		...
		println("Processing frames $frame to $(min(frame + nImages - 1, sfm_end)) : size = ($height, $(Int32(width//imgWidth)) * $imgWidth)")
		...
		CUDA.synchronize()
        save("assets/sfm/filtered_blobs_$(frame)_$(min(frame + nImages - 1, sfm_end)).png", colorview(RGBA, Array(blank_slate)))
